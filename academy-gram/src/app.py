...
from flask import Flask, request, session, redirect, url_for, render_template, g, flash, abort, make_response, send_from_directory, Response
...
class Post:
    ...
    def get_all():
        db = get_db()
        cur = db.execute('SELECT p.content, p.image_path, u.username FROM posts p JOIN users u ON p.user_id = u.user_id WHERE p.is_private = 0 ORDER BY p.pub_date DESC')
        return cur.fetchall()

    @staticmethod
    def get_for_user(user_id):
        db = get_db()
        cur = db.execute('SELECT content, image_path, is_private FROM posts WHERE user_id = ? ORDER BY pub_date DESC', [user_id])
        return cur.fetchall()
    
    @staticmethod
    def create(user_id, content, image_path=None, is_private=False):
        db = get_db()
        db.execute('INSERT INTO posts (user_id, content, image_path, is_private) VALUES (?, ?, ?, ?)',
...
class PasswordReset:
    ...
    def create(user_id):
        ...
        db.execute('INSERT INTO password_resets (user_id, reset_code, expires_at) VALUES (?, ?, ?)',
    ...
    def find(user_id, code):
        db = get_db()
        cur = db.execute('SELECT * FROM password_resets WHERE user_id = ? AND reset_code = ? ORDER BY expires_at DESC LIMIT 1',
                         [user_id, code])
        return cur.fetchone()
...
def forgot_password():
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            flash(f"A password reset code has been generated for {user['username']}. For the purpose of this demo, the code is {code}.")
...
def reset_password(username):
    ...
    if request.method == 'POST':
        ...
        if not reset_token:
            error = "Invalid code."
        elif datetime.strptime(reset_token['expires_at'], '%Y-%m-%d %H:%M:%S.%f') < datetime.utcnow():
...
def interests():
    ...
    # Vulnerable: user_id is taken directly from the query parameter
    user_id = request.args.get('user_id', g.user['user_id'])
    
    # No authorization check to see if the logged-in user is allowed to see this
    user_of_interest = User.find_by_id(user_id)
    
    if not user_of_interest:
        abort(404)
        
    return render_template('interests.html', user_of_interest=user_of_interest)
...
# --- Internal API for Checker ---
@app.route('/api/update_interests', methods=['POST'])
def update_interests():
    data = request.json
    if not data or 'user_id' not in data or 'interests' not in data:
        abort(400)
    
    # For security, only allow updating the admin user (user_id=1)
    if data['user_id'] != 1:
        abort(403)

    db = get_db()
    db.execute('UPDATE users SET interests = ? WHERE user_id = ?', [data['interests'], data['user_id']])
    db.commit()
