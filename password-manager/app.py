from flask import Flask, request, jsonify, session, send_file, render_template, redirect, url_for, flash
import io
import csv
from database import Database
from vault_storage import VaultManager
from crypto_utils import CSVFormatter
from config import Config

app = Flask(__name__)
app.config.from_object(Config)

# Initialize database and vault manager
db = Database()
...
def validate_master_password(master_password):
    ...
    if len(master_password) < Config.MASTER_PASSWORD_MIN_LENGTH:
        return False, f"Master password must be at least {Config.MASTER_PASSWORD_MIN_LENGTH} characters"
    return True, ""
...
def api_create_vault():
    ...
    try:
        vault_id = vault_manager.create_vault(user['id'], vault_name, entries, master_password)
        return jsonify({
...
def add_vault_entries(vault_id):
    ...
    try:
        count = vault_manager.add_entries_to_vault(vault_id, user['id'], entries, master_password)
        return jsonify({
            'message': f'Added {count} entries to vault',
            'entries_added': count
        })
    except ValueError as e:
        return jsonify({'error': str(e)}), 400
...
def import_csv(vault_id):
    ...
    try:
        ...
        count = vault_manager.add_entries_to_vault(vault_id, user['id'], entries, master_password)
        return jsonify({
            'message': f'Imported {count} entries from CSV',
...
def download_vault(vault_id):
    ...
    try:
        encrypted_data, filename = vault_manager.download_vault(vault_id, master_password, comment)
...
def download_vault_page(vault_id):
    ...
    if request.method == 'POST':
        ...
        try:
            encrypted_data, filename = vault_manager.download_vault(vault_id, master_password, comment)
            file_obj = io.BytesIO(encrypted_data)
            return send_file(file_obj, as_attachment=True, download_name=filename, mimetype='application/octet-stream')
        except ValueError as e:
            flash(str(e), 'error')
...
def import_csv_page():
    """Import CSV to create new vault"""
    user = require_auth()
    if not user:
        flash('Please login first', 'error')
        return redirect(url_for('login_page'))
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            vault_id = vault_manager.create_vault(user['id'], vault_name, entries, master_password)
            flash(f'Created vault "{vault_name}" with {len(entries)} entries', 'success')
            return redirect(url_for('vault_view', vault_id=vault_id))
        except Exception as e:
            flash(f'Failed to import CSV: {str(e)}', 'error')
            return render_template('import_csv.html')
...
def browse_vaults():
    """Browse all public vaults"""
    # Get all vaults with user information
...
def public_download_vault(vault_id):
    """Download any vault (public access)"""
