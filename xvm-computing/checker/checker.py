...
class XVMComputingChecker(checkerlib.BaseChecker):
    ...
    XASM_PATH: str = os.path.join("/opt/checker/xvm-computing", "src", "build", "xasm")
    ...
    def place_flag(self, tick: int) -> checkerlib.CheckResult:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"[DEBUG] place_flag: failed to import flag_templates: {e}",
                    flush=True,
                )
            ...
            if len(bin_bytes) >= self.MAX_UPLOAD:
                print(
                    f"[DEBUG] place_flag: binary too large: {len(bin_bytes)} bytes",
                    flush=True,
                )
                return checkerlib.CheckResult.FAULTY
    ...
    def check_flag(self, tick: int) -> checkerlib.CheckResult:
        ...
        print(
            f"[DEBUG] check_flag: mismatch expected={expected!r} got={first_line!r}",
            flush=True,
        )
        return checkerlib.CheckResult.FLAG_NOT_FOUND
    ...
    def _upload_and_execute(
        self, bin_bytes: bytes, input_bytes: Optional[bytes] = None
    ) -> Optional[bytes]:
        ...
        try:
            ...
            print(
                f"[DEBUG] _upload_and_execute: connect upload failed: {e}", flush=True
            )
            return None
        try:
            token = self._upload_xvm(r, bin_bytes)
            if not token:
                print(
                    "[DEBUG] _upload_and_execute: upload returned empty token",
                    flush=True,
                )
                return None
        finally:
        ...
        try:
            ...
            if input_bytes:
                r.send(input_bytes)
            data = r.recvuntil(
                b"================================", timeout=self.IO_TIMEOUT_S
            )
            out = (data.split(b"================================", 1)[0]).replace(
                b"\r", b""
            )
            return out
        except Exception as e:
    ...
    def _test_menu_integrity(self) -> checkerlib.CheckResult:
        ...
        try:
            ...
            try:
                ...
                print(
                    f"[DEBUG] _test_menu_integrity: import service_checks failed: {e}",
                    flush=True,
                )
                return checkerlib.CheckResult.FAULTY
            if expected.split(b"\n")[0] not in menu:
    ...
    def _test_xinfo_basic(self) -> checkerlib.CheckResult:
        ...
        try:
            ...
            if not token:
                print(
                    "[DEBUG] _test_xinfo_basic: upload returned empty token", flush=True
                )
                return checkerlib.CheckResult.FAULTY
        finally:
            try:
        ...
        try:
            ...
            if not info_text:
                ...
                return checkerlib.CheckResult.FAULTY
            if (
                b"Dumping Section Info" not in info_text
                or b"Dumping SymTab" not in info_text
            ):
    ...
    def _connect(self):
        ...
        if not host:
            ...
            host = (
                checkerlib.get_service_address(self.team)
                if hasattr(checkerlib, "get_service_address")
                else None
            )
        if not host:
